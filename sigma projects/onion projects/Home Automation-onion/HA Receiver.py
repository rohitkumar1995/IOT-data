...
References :
    ...
    SQLite3 - https://www.sqlite.org/index_ws.html     (lightweight relational database)
...
def toDatabase(data):
    try:
        ...
        cur.execute("INSERT INTO dataTable_losant (new_state, timestamps) VALUES ('%s','%s')"%(data['state'],data['timestamp']))
...
def al():
    ...
    cur.execute("select * from dataTable_losant")  # SQL query for fetching all of the data from the table
...
def index():
    ...
    return render_template('index1.html', rows = rows)   # rendering the HTML page and passing the fetched values

# function to subscribe to the topic
@socketio.on('subscribe')
def handle_subscribe():
    #topic = 'sigmaway/akrd/db1'     # topic name, must be same as the topic name of publisher
    topic = 'sigmaway/losant'     # topic name, must be same as the topic name of publisher
    qos = 0
...
def clearData():
    try:
        ...
        cur = conn.cursor()
        cur.execute("DELETE FROM dataTable_losant")
        cur.execute("DELETE FROM sqlite_sequence WHERE name = 'dataTable_losant';")    # resetting the autoincrement field
        conn.commit()
        cur.close()
        conn.close()
